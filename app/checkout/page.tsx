...
const CheckoutPage = () => {
  ...
  // Create WooCommerce order for COD
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (response: any) {
          // 3. Verify payment in your backend
          ...
          // 4. Create WooCommerce order with payment marked complete
          const createOrderRes = await fetch("/api/checkout/create-order", {
            ...
            body: JSON.stringify({
              ...
              paymentDetails: response, // Pass Razorpay payment info to backend
            }),
          });
          ...
          if (createOrderData.success) {
            window.location.href = createOrderData.checkoutUrl;
          } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Address object in WooCommerce REST API format
    const address = {
      billing: {
        first_name: formData.firstName,
        last_name: formData.lastName,
